...
import { getFailComment } from '../../../src/lifecycles/fail/get-fail-comment';

test('Comment with mutiple errors', () => {
  const errors = [
  ...
  expect(comment).toMatch(/the `master` branch/);
...
test('Comment with missing error details and pluginName', () => {
...
test('Comment with missing error details and no pluginName', () => {
  const error = new SemanticReleaseError('Error message 1', 'ERR1');
  const errors = [error];
  const comment = getFailComment({ name: 'master' }, errors);
